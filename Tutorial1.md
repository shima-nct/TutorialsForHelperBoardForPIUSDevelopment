...
VESCのとモーターとの接続には、VESCのバリエーションによりますが、4mmもしくは5mm bullet connectorを使用します。このbullet connecotorは明確な規格が定められていません。どのメーカーの製品が適合するかは不明です。調達にあたっては事前調査を行う必要があります。なお、VESCハードウェアの公式ショップでは適合するbullet connectorのプラグ（オス）が販売されています。
...
1. グローバルスコープに置かれた要素数10の配列dataの値から求めた平均値を返すメソッドaverageOfArrayを書きなさい。
...
SerialPlotを起動したら下ペインの`Port`タブでシリアルポートを選択し、ボーレートを設定してください。
...
その他のRTOSでよく用いられているものをいくつか調べ、それぞれどのような分野で使われているのかを調べ、答えなさい。

### 3.4

Zephyr RTOSについて調べ、どのような分野で使われているのか、またその特徴を調べて答えなさい。